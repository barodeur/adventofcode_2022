...
let () =
  ...
  in
  let divisors_product =
    Array.fold monkeys ~init:1 ~f:(fun product monkey ->
  ...
  |> Sequence.take_while ~f:(fun (round, _) -> round <= 10000)
  |> Sequence.iter ~f:(fun (_, monkey_idx) ->
         ...
         List.iter items ~f:(fun wlevel ->
             ...
             let new_wlevel = monkey.operation wlevel % divisors_product in
